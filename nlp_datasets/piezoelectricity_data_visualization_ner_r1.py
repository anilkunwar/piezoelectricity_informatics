...
class Config:
    ...
    # Database paths — now pointing into the knowledge_database subdirectory
    DB_PATHS = {
...
def main():
    ...
    with st.sidebar:
        ...
        if not available_dbs:
            ...
            st.info("""
            Please ensure the database files are in the `knowledge_database/` folder:
            - piezoelectricity_metadata.db
            - piezoelectricity_reliability_universe.db
    ...
    if st.session_state.analysis_complete:
        ...
        texts = data['texts']
        all_entities = {"materials": [], "properties": [], "quantities": []}
        # Reconstruct all_entities for histogram
        for _, row in entities_df.iterrows():
            ...
            elif row['type'] == 'quantity':
                # Not stored in original; will be filled from relationships below
                pass
        # Fill quantities from relationships for histogram use
        quantities = []
        ...
        if "Histograms" in viz_options:
            with tabs[4]:
                ...
                quantities = all_entities['quantities']
                if quantities:
