...
# ==============================
# DATABASE MANAGER — CORE OF CRASH RESISTANCE
# ==============================
class DatabaseManager:
    def __init__(self):
        self.metadata_db = METADATA_DB_FILE
        self.universe_db = UNIVERSE_DB_FILE
    ...
    def init_databases(self):
        ...
        conn.commit()
        conn.close()
        update_log(f"Initialized metadata database at {self.metadata_db}")
        ...
        conn.commit()
        conn.close()
        update_log(f"Initialized universe database at {self.universe_db}")

        # PDF Storage DB — BLOB storage
        ...
        conn.commit()
        conn.close()
        update_log(f"Initialized PDF storage database at {self.pdf_db}")

    def store_pdf_data(self, paper_id: str, pdf_bytes: bytes) -> bool:
        """Store PDF in dedicated database with SHA256 deduplication."""
        try:
            ...
            existing = c.fetchone()
            if existing:
                update_log(f"PDF hash duplicate for {paper_id}; skipping storage")
                # Still mark as stored in metadata
                c.execute("UPDATE papers SET pdf_stored = 1 WHERE id = ?", (paper_id,))
            else:
                c.execute("""INSERT OR REPLACE INTO pdf_storage
                    (paper_id, pdf_data, pdf_hash, file_size)
                    VALUES (?, ?, ?, ?)""",
                    (paper_id, sqlite3.Binary(pdf_bytes), pdf_hash, file_size))
                update_log(f"Stored new PDF for {paper_id} ({file_size} bytes)")
    ...
    def get_pdf(self, paper_id: str) -> Optional[bytes]:
        """Retrieve PDF bytes from database."""
        try:
            conn = sqlite3.connect(self.pdf_db)
            c = conn.cursor()
            ...
            if result:
                ...
                update_log(f"No PDF found in DB for {paper_id}")
                return None
        except Exception as e:
            update_log(f"Error retrieving PDF for {paper_id}: {e}")
            return None

    def has_pdf(self, paper_id: str) -> bool:
        """Check if PDF exists in storage DB."""
        try:
            conn = sqlite3.connect(self.pdf_db)
            c = conn.cursor()
            c.execute("SELECT 1 FROM pdf_storage WHERE paper_id = ? LIMIT 1", (paper_id,))
            exists = c.fetchone() is not None
            conn.close()
            return exists
        except Exception as e:
            update_log(f"Error checking PDF existence for {paper_id}: {e}")
            return False

    def create_zip_from_db(self, paper_ids: List[str]) -> io.BytesIO:
        """Create ZIP archive from PDF database."""
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
                for paper_id in paper_ids:
                    ...
                    if pdf_data is not None:
                        zipf.writestr(f"{paper_id}.pdf", pdf_data)
                    else:
                        update_log(f"Skipping {paper_id} in ZIP — PDF not found")
            zip_buffer.seek(0)
...
def save_session_state():
    """Save session state — only paper IDs, no binary data."""
    try:
        session_data = {
...
<div class="card fade-in">
    ...
    <ul>
        ...
        <li><strong>PDFs</strong> in a dedicated <code>piezoelectricity_pdfs.db</code> (BLOB storage)</li>
    </ul>
    <div style="background: linear-gradient(135deg, #DCFCE7 0%, #BBF7D0 100%); padding: 1rem; border-radius: 8px; border-left: 4px solid #10B981;">
        <strong>✅ Crash-Resistant Design:</strong> No PDF bytes in memory — stored in SQLite BLOB.
    </div>
</div>
""", unsafe_allow_html=True)
...
if IS_CLOUD:
    st.warning("""
    ☁️ **Streamlit Cloud Mode**: Files are stored temporarily in `/tmp`. 
    ...
    try:
        ...
        for result in client.results(search):
            ...
            papers.append({
                ...
                "downloaded": db_manager.has_pdf(paper_id)  # Check DB, not session
            })
...
def handle_pdf_download(paper_id, pdf_url, paper_metadata):
    """Download PDF, store in DB, extract text, and update databases."""
    try:
        # Step 1: Download PDF
        pdf_bytes = download_pdf_bytes(pdf_url)
        update_log(f"Downloaded PDF for {paper_id} ({len(pdf_bytes)} bytes)")
        
        # Step 2: Store in PDF DB
        if not db_manager.store_pdf_data(paper_id, pdf_bytes):
        ...
        for page in doc:
            try:
                ...
                update_log(f"Skipping unparseable page in {paper_id}: {e}")
        doc.close()
        update_log(f"Extracted {len(full_text)} characters from {paper_id}")
        
        # Step 4: Update universe DB
        ...
        # Step 5: Update session state (only ID)
        ...
        save_session_state()
        update_log(f"Successfully indexed {paper_id}")
        return True
    except Exception as e:
...
def create_zip_of_downloaded_pdfs():
    """Create a ZIP file of all downloaded PDFs from database."""
    if not st.session_state.downloaded_pdfs:
        return None
    return db_manager.create_zip_from_db(st.session_state.downloaded_pdfs)
...
def show_database_stats():
    ...
    if stats:
        ...
        with col3:
            st.metric("Full-Text Papers", stats.get('fulltext_count', 0))
        with col4:
            st.metric("Total PDF Size", f"{stats.get('total_pdf_size_mb', 0):.1f} MB")
        if stats.get('total_papers', 0) > 0:
            coverage = (stats.get('pdfs_stored', 0) / stats.get('total_papers', 1)) * 100
            st.progress(coverage / 100, text=f"PDF Coverage: {coverage:.1f}%")
                ...
                for idx, paper in df.iterrows():
                    ...
                    *({paper['year']})* — **{paper['relevance_prob']}%** relevance
                    """):
                        ...
                        with col_save:
                            if db_manager.has_pdf(paper["id"]):
                                ...
                                st.button("💾 Not Downloaded", disabled=True, use_container_width=True)
