...
def normalize_text(text):
    ...
    # Superscripts (not heavily used, but included for completeness)
    superscripts = {
        '⁰': '0', '¹': '1', '²': '2', '³': '3', '⁴': '4',
        '⁵': '5', '⁶': '6', '⁷': '7', '⁸': '8', '⁹': '9'
    }
            ...
            if not relevant_papers:
                ...
                for idx, paper in df.iterrows():
                    with st.expander(f"📄 **{paper['title']}** ({paper['year']}) — **{paper['relevance_prob']}%**"):
                        ...
                        col_btn, col_links, col_view = st.columns([1, 1, 2])
                        with col_btn:
                            if st.button("📥 Download & Index PDF", key=f"download_{paper['id']}"):
                        ...
                        with col_view:
                            # ✅ CRITICAL: Individual PDF download button
                            if paper["id"] in st.session_state.downloaded_pdfs:
                                st.download_button(
                                    ...
                                    key=f"save_pdf_{paper['id']}"
                                )
                        with col_links:
                            abs_url = paper['pdf_url'].replace('/pdf/', '/abs/')
                            st.markdown(f"[🌐 View on arXiv]({abs_url})")
