...
def main():
    ...
    with st.sidebar:
        ...
        if not available_dbs:
            st.error(f"No databases found for query '{st.session_state.current_query_id}' in `knowledge_database/`!")
        ...
        analyze_enabled = len(available_dbs) > 0
    ...
    if analyze_btn and available_dbs:
        with st.spinner(f"🔬 Analyzing dopant relationships from Query {st.session_state.current_query_id} literature..."):
            try:
                ...
                # Load papers
                st.text(f"📥 Loading papers from database for Query {st.session_state.current_query_id}...")
                ...
                if relationships_df.empty:
                    ...
                    with st.expander("🔍 Sample Papers for Debugging"):
                        st.markdown(f"Here are some sample papers from Query {st.session_state.current_query_id}. Check if they contain dopant information:")
                    ...
                    with st.expander("📊 Analysis Summary"):
                        ...
                        col2.metric("Unique Dopants", relationships_df['dopant'].nunique())
                        col3.metric("Base Materials", relationships_df['base_material'].nunique())
                        ...
                        fig = px.bar(
                            ...
                            title=f'Most Studied Dopant Categories (Query {st.session_state.current_query_id})',
                ...
                st.error(f"Analysis failed for Query {st.session_state.current_query_id}: {str(e)}")
                logger.error(f"Analysis failed for Query {st.session_state.current_query_id}: {str(e)}", exc_info=True)
                return
    ...
    if st.session_state.dopant_relationships is not None and not st.session_state.dopant_relationships.empty:
        ...
        with tabs[0]:
            ...
            with col1:
                fig = engine.create_sunburst_chart(relationships_df, f"Dopant Impact Hierarchy for Query {st.session_state.current_query_id}")
                if fig:
                    st.plotly_chart(fig, use_container_width=True)
                    add_caption(r"""
            ...
            if len(selected_dopants) < 2:
                ...
                with col1:
                    fig = engine.create_radar_chart(relationships_df, selected_dopants, 
                                                  f"Multi-Property Performance Comparison (Query {st.session_state.current_query_id})")
                    if fig:
                        st.plotly_chart(fig, use_container_width=True)
                        add_caption(r"""
            ...
            if st.button("✨ Generate Recommendations", type="primary"):
                with st.spinner("Generating optimized dopant recommendations..."):
                    ...
                    if not recommendations:
                        ...
                        st.markdown(f"### 🏆 Top Recommendations for {application} (Query {st.session_state.current_query_id})")
                        ...
                        st.markdown(f"""
                        **For {application} applications (Query {st.session_state.current_query_id}):**
        ...
        with tabs[4]:
            ...
            st.markdown(f"### Extracted Dopant Relationships (Query {st.session_state.current_query_id})")
            ...
            with col1:
                ...
                st.download_button("📥 Download CSV", csv, f"dopant_relationships_{st.session_state.current_query_id}.csv", "text/csv")
            ...
            with col2:
                ...
                excel_buffer.seek(0)
                st.download_button("📊 Download Excel", excel_buffer, f"dopant_analysis_{st.session_state.current_query_id}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            
            with col3:
                json_data = relationships_df.to_dict('records')
                json_str = json.dumps(json_data, indent=2)
                st.download_button("💾 Download JSON", json_str, f"dopant_data_{st.session_state.current_query_id}.json", "application/json")
        ...
        with st.expander("🚀 How It Works"):
            ...
            ```
            knowledge_database/
            ├── piezoelectricity_metadata.db          # Default (q0)
            ├── piezoelectricity_universe.db
            ├── piezoelectricity_pdfs.db
            ├── piezoelectricityq1_metadata.db        # Query 1
            ├── piezoelectricityq1_universe.db
            ├── piezoelectricityq1_pdfs.db
            ├── piezoelectricityq2_metadata.db        # Query 2
            ├── piezoelectricityq2_universe.db
            └── piezoelectricityq2_pdfs.db
            ```
        ...
        if st.checkbox(f"✅ Use sample data for Query {st.session_state.current_query_id}"):
            with st.spinner(f"Generating sample dataset for Query {st.session_state.current_query_id}..."):
                relationships_df, focus_area = create_sample_data_for_query(st.session_state.current_query_id)
                st.session_state.dopant_relationships = relationships_df
                st.success(f"✅ Sample data for Query {st.session_state.current_query_id} ready! Focus: {focus_area}")
                st.rerun()
...
if __name__ == "__main__":
    # Initialize logging
    logger.info("Application started with query dataset support")
